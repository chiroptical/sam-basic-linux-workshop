...
\begin{frame}{Text Editors}
    \begin{itemize}
        \item Use which one you like (\texttt{vim, emacs, \color{pittgold}nano\color{black}, gedit}) 
...
\vfill
\mybutton{exercises-1}{Exercises (1)} \mybutton{exercises-2}{Exercises (2)} \mybutton{exercises-3}{Exercises (3)}
\end{frame}
...
\begin{frame}[label=navigate]{Navigate the file system}
...
\vfill
\mybutton{exercises-2}{Exercises (2)} \mybutton{exercises-3}{Exercises (3)} \mybutton{exercises-4}{Exercises (4)}
\end{frame}
...
\vfill
\mybutton{exercises-3}{Exercises (3)} \mybutton{exercises-4}{Exercises (4)} \mybutton{exercises-6}{Exercises (6)}
\end{frame}
...
\begin{itemize}
    ...
    \myvbutton{exercises-4}{Exercises (4)}
\end{itemize}
\end{frame}
...
\vfill
\mybutton{exercises-4}{Exercises (4)} \mybutton{exercises-5}{Exercises (5)}
\end{frame}
...
\begin{itemize}
    \item Southern Rock Band, \texttt{y} is their vowel of choice.
    \item Replace all of the vowels in \texttt{doctored.txt} with \texttt{y}
\end{itemize}
\inputminted[bgcolor=lightgray,linenos,fontsize=\footnotesize]{bash}{code/simple-filters-4.txt}
\begin{itemize}
    \item Try this filter on \texttt{names.txt}, is the behavior expected?
\end{itemize}
\end{frame}
...
\myvbutton{exercises-6}{Exercises (6)}
\end{frame}
...
\begin{frame}[label=exercises-4]{Exercises (4)}
    \begin{itemize}
        ...
        \end{itemize}
        \item Make \texttt{hello.py} executable and prepend directory to \texttt{\$PATH} (\mybutton{permissions}{hint})
        \begin{itemize}
            \item Be careful not to overwrite current \texttt{\$PATH}!
        \end{itemize}
        \item Output from \texttt{ls} is colored, why? (try \texttt{which ls})
        \begin{itemize}
            \item use an alias to make \texttt{grep}'s behavior the same (\mybutton{alias}{hint})
        \end{itemize}
...
\begin{itemize}
    \item Use \texttt{tr} to remove extraneous whitespace from \texttt{states.txt} (\mybutton{sort-uniq-tr}{hint})
    \item Add 1 to the 3rd column of \texttt{random.txt} using \texttt{awk} (\mybutton{grep-awk-sed}{hint})
    \item All of the ``New'' states have decided they are now ``Relatively New''.
    \begin{itemize}
        \item replace ``New'' with ``Relatively New'' using \texttt{sed} (\mybutton{grep-awk-sed}{hint})
        \item (More difficult) Try using capture groups (hint: Try \texttt{echo "hello" |
...
\begin{frame}[label=difficult]{Exercises (Difficult 2)}
\begin{itemize}
    \item Let's revisit the Lynyrd Skynyrd algorithm. We want our algorithm to
    work on both \texttt{names.txt,doctored.txt} but \texttt{sed} lacks
    a feature to do it easily (take my word for it). We need
    \texttt{perl}'s ``lookbehind'' functionality; read the first 4 sections of
    \url{http://www.regular-expressions.info/lookaround.html}. After, ask youself:
    \begin{itemize}
        \item What was wrong with the \texttt{sed} command? (\mybutton{lynyrd}{hint})
        \begin{itemize}
            \item e.g. apply the filter to ``erin'' vs ``ken'', we want ``eryn'' and ``kyn''
        \end{itemize}
        \item Looking at the above example, what should be behind the vowels we want to replace?
    ...
    \item \mybutton{perl-helper}{hint}
\end{itemize}
\end{frame}

\begin{frame}[label=perl-helper]{Perl Helper}
\begin{itemize}
    \item Try using \url{https://www.debuggex.com/cheatsheet/regex/pcre}
    \item Look in the ``Regular Expression Character Classes'' section
\end{itemize} 
\myvbutton{difficult}{Exercises (Difficult 2)}
\end{frame}
